...
def main():
    ...
    print(f"Total viruses on website: {len(viruses)}")
    
    try:
        df = pd.read_csv(OUTPUT_FILE)
        scraped_viruses = df['Virus'].unique()
        print(f"Total unique viruses in CSV: {len(scraped_viruses)}")
        
        missing = []
        for virus_name, code in viruses:
            if virus_name not in scraped_viruses:
                missing.append((virus_name, code))
                
        print(f"\nMissing {len(missing)} viruses:")
        for v in missing:
            print(f"- {v[0]} ({v[1]})")
